import { Col, Drawer, Grid, Image, Row, Space, Typography } from "antd";
import React, { useEffect, useRef, useState } from "react";
import { SiBrandfolder } from "react-icons/si";
import {
  BsArrowLeftSquareFill,
  BsArrowRightSquareFill,
  BsPersonSquare,
} from "react-icons/bs";
import { RiFacebookBoxFill, RiInstagramFill } from "react-icons/ri";
import "./supplier-profile-drawer-module.css";

const styles = {
  parent: {
    borderRadius: "7px",
    border: "1px solid #d8dee4",
    background: "#f6f8fa",
  },
};

const { Text, Title } = Typography;

const SupplierProfileBanner = () => {
  return (
    <div className="supplierProfileDrawer__banner">
      <Image
        width={"100%"}
        preview
        height={170}
        style={{ objectFit: "cover", objectPosition: "center" }}
        src="https://thumbs.dreamstime.com/b/web-banner-mega-sale-template-your-business-project-geometric-design-random-polygonal-shapes-d-style-vector-web-169671561.jpg"
        fallback="data:image/png;base64,"
      />
      <Image
        wrapperStyle={{ position: "absolute", bottom: -30, left: 30 }}
        width={100}
        height={100}
        style={{ objectFit: "cover" }}
        src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRZpUZyS8yIatbyOvFBX3GV7QqUleB-Oxa8WCWZVbSFaOBv1BjMckjmLhQIdj5WUkfph0A&usqp=CAU"
        fallback="data:image/png;base64,"
      />
    </div>
  );
};

const SupplierProfileDetails = () => {
  return (
    <Row gutter={16} style={{ height: "max-content" }}>
      <Col span={12}>
        <div className="w-100 p-3" style={{ height: "100%", ...styles.parent }}>
          <Title level={4} className="d-flex align-items-center ">
            <SiBrandfolder className="me-2" />
            Brand Details
          </Title>
          <Text>
            <b>Brand Name : </b>
            Sunsilk
          </Text>
          <br />
          <Text>
            <b>Owner Name : </b>
            Muhmmad Shahzaib Alam
          </Text>
        </div>
      </Col>

      <Col span={12}>
        <div className="w-100 p-3 h-100" style={{ ...styles.parent }}>
          <Title level={4} className="d-flex align-items-center ">
            <BsPersonSquare className="me-2" />
            Supplier Details
          </Title>
          <Text>
            <b>Firstname : </b>
            Muhammad Shahzaib
          </Text>
          <br />
          <Text>
            <b>Lastname : </b>
            Alam
          </Text>
          <br />
          <Text>
            <b>Phone no # </b>
            92-3485604453
          </Text>
          <br />

          <Space direction="horizontal" className="my-2">
            <RiInstagramFill
              size={20}
              fill="#C13584"
              style={{ cursor: "pointer" }}
            />
            <RiFacebookBoxFill
              size={20}
              fill="#4267B2"
              style={{ cursor: "pointer" }}
            />
          </Space>
        </div>
      </Col>
    </Row>
  );
};

const SupplierProfileAboutMe = () => {
  const [isMoreDetails, setIsMoreDetails] = useState(false);
  const handleMoreDetais = () => {
    setIsMoreDetails((old) => !old);
  };
  return (
    <div className="my-3 p-3" style={{ ...styles.parent }}>
      <Title level={4} className="d-flex align-items-center ">
        <BsPersonSquare className="me-2" />
        About Me
      </Title>
      <Text className="">
        Lorem ipsum dolor sit amet consectetur adipisicing elit. At ratione
        Lorem ipsum dolor sit amet consectetur, adipisicing elit. Neque, saepe.{" "}
        {isMoreDetails &&
          `Lorem, ipsum dolor sit amet consectetur adipisicing elit.
          Similique dicta ut architecto repellendus, laudantium dolor minima
          asperiores rem consequatur sed. Est dolorum optio iusto itaque illum
          quo minima incidunt pariatur.`}
        <b onClick={handleMoreDetais} className="ms-1">
          <u>{isMoreDetails ? "Hide Details" : "More Details ..."}</u>
        </b>
      </Text>
      <br />
    </div>
  );
};

const SupplierProfileProducts = () => {
  const sliderRef = useRef(document.getElementById("Slider__Ref"));

  useEffect(() => {
    const sliderRefCopy = sliderRef.current;

    const handleScroll = (evt) => {
      sliderRefCopy.classList.remove("smooth-scroll")
      evt.preventDefault();
      console.log(evt.deltaY);
      sliderRefCopy.scrollLeft += evt.deltaY;
    };

    sliderRefCopy.addEventListener("wheel", handleScroll);

    return () => {
      if ((sliderRefCopy === null) | undefined) return;
      sliderRefCopy.removeEventListener("wheel", handleScroll);
    };

    // eslint-disable-next-line
  }, [sliderRef.current]);

  const slideLeft = (e) => {
    sliderRef.current.classList.add("smooth-scroll")

    sliderRef.current.scrollLeft -= 300;
  };

  const slideRight = (e) => {
    sliderRef.current.classList.add("smooth-scroll")

    sliderRef.current.scrollLeft += 300;
  };

  return (
    <div className="my-3 p-3 position-relative" style={styles.parent}>
      <Title level={4} className="d-flex align-items-center ">
        <BsPersonSquare className="me-2" />
        Hot Selling Products
      </Title>
      <div className="slider__arrow arrow-left  py-3 px-2" onClick={slideLeft}>
        <BsArrowLeftSquareFill size={20} />
      </div>
      <Row
        gutter={8}
        ref={sliderRef}
        className="mx-0 py-2 position-relative overflow-hidden smooth-scroll"
        id="Slider__Ref"
        wrap={false}
      >
        {[...Array(9)].map((_, idx) => (
          <Col key={idx}>
            <div>
              <Image
                width={100}
                height={100}
                style={{ objectFit: "cover" }}
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRZpUZyS8yIatbyOvFBX3GV7QqUleB-Oxa8WCWZVbSFaOBv1BjMckjmLhQIdj5WUkfph0A&usqp=CAU"
                fallback="data:image/png;base64,"
              />
            </div>
          </Col>
        ))}
      </Row>
      <div className="slider__arrow arrow-right py-3 px-2" onClick={slideRight}>
        <BsArrowRightSquareFill size={20} />
      </div>
    </div>
  );
};

const { useBreakpoint } = Grid;

function SupplierProfileDrawer({
  visible = false,
  setIsVisible,
  supplierId = null || 0,
}) {
  const { lg } = useBreakpoint();
  if (supplierId === null || undefined) return;

  return (
    <Drawer
      bodyStyle={{ paddingBottom: "30px" }}
      width={lg ? 640 : "100%"}
      placement="right"
      onClose={() => setIsVisible(false)}
      visible={visible}
    >
      <SupplierProfileBanner />
      <SupplierProfileDetails />
      <SupplierProfileAboutMe />
      <SupplierProfileProducts />
    </Drawer>
  );
}

export default SupplierProfileDrawer;
