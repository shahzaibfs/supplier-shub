import { Grid, Image, Row, Typography } from "antd";
  ...
  return (
    ...
    >
      {categories.map((category, idx) => (
        <CategoryCard categoryData={category} key={idx}/>
      ))}
    </Row>
  );
}
...
const CategoryCard = ({categoryData={}}) => {
  if(Object.keys(categoryData).length <=0) return <Text type="danger">Network Err : Please Refresh The Browser Later</Text>
    ...
    <div >
      ...
      >
        <Image
          width={80}
          height={80}
          src={categoryData.categoryPhotoUrl}
        ...
        />
        <Text className="mt-2 text-center">{categoryData.categoryName}</Text>
      </div>
    </div>
  );
};
